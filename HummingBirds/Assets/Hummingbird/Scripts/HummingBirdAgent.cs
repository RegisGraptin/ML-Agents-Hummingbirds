...
public class HummingBirdAgent : Agent
{
   ...
   public bool trainingMode;
    ...
    public override void OnEpisodeBegin()
    {
        ...
        if (trainingMode)
        {
            // Spawn in front of a flower with 50% of chance
    ...
    /// <summary>
    /// Called when an action is received from either player or neural network input.
    /// 
    /// actions represents:
    ...
    public override void OnActionReceived(ActionBuffers actions)
    {
        ...
        // Limit the pitch value to avoid bird flipping 
    ...
    public override void CollectObservations(VectorSensor sensor)
    {
        ...
        // Observe the angent's local rotation (4 observations)
        ...
        // +1 => beak tip is directly in fornt of the flower, -1 means directly behind
        ...
        // +1 => beak pointing directly at the flower, -1 means directly away
        sensor.AddObservation(
    ...
    /// <see cref="OnActionReceived(ActionBuffers actions)"/> instead of using the neural network.
    /// </summary>
    ...
    {
        Debug.Assert(trainingMode == false, "Frezze/Unfreeze not supported in training mode");
        frozen = true;
        rigidbody.Sleep();
    }
    ...
    /// <summary>
    /// Move the agent to a safe random position (i.e. dos not collide with anything)
    ...
    private void MoveToSafeRandomPosition(bool inFrontOfFlower)
    {
        ...
        while (!safePositionFound && attemptsRemaining > 0)
        {
            ...
            {
                // Pick a random height of the ground
    ...
    private void UpdateNearestFlower()
    {
        foreach (Flower flower in flowerArea.Flowers)
        {
            ...
            {
                // Calculate the distance to this flower and distance to the current nearest flower
    ...
    /// <summary>
    /// Handles when the agent's collider enter or stay in a trigger collider
    /// </summary>
    /// <param name="other">The trigger collider</param>
    private void TriggerEnterOrStay(Collider collider) 
    {
        ...
        if (collider.CompareTag("Nectar"))
        {
            ...
            if (Vector3.Distance(beakTip.position, closestPointToBeakTip) < BeakTipRadius)
            {
                ...
                // Attempts to take 0.01 nectar
                // Note: this is per fixed timestep, every 0.02 seconds
    ...
    /// 
    /// Punish the agent when it touches the boundary or the ground.
    /// </summary>
    ...
    /// 
    /// Avoirs scenario where nearest flower is stolen by opponent and not updated
    /// </summary>
